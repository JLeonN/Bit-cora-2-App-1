...
- [ ] Crear `src/components/Modales/ModalDestinoExcelCompartido.vue` con responsabilidad exclusivamente visual.
...
- [ ] Reemplazar el comportamiento automático de `MainLayout.vue` por una decisión explícita.
...
- [ ] Agregar en `MainLayout.vue` las tres acciones del modal.
...
- [ ] Integrar el modal con el estado global y el botón Atrás.
...
- [ ] Impedir que `SelectorExcel.vue` consuma cualquier pendiente sin autorización.
...
- [ ] Autorizar el destino maestro desde `AjustarUbicaciones.vue`.
...
- [ ] Preparar la entrega al destino Listados sin consumir aún el archivo.
...
- [ ] Verificar que no se requieren cambios nativos.
...
- [ ] Implementar `procesarExcelCompartidoPendiente()` para el destino externo.
...
- [ ] Implementar el ciclo de vida del pendiente externo en `PaginaListados.vue`.
...
- [ ] Crear `ejecutarImportacionExcel({ obtenerResultado, identificadorCompartido = '' })` como orquestador único.
...
- [ ] Crear `aplicarResultadoImportacionExcel(resultadoAnalisis, { identificadorCompartido = '' } = {})` como única función de inserción y resumen.
...
- [ ] Después de completar lint, pruebas y build, ejecutar el flujo Android oficial para sincronizar la WebView, generar el AAB y abrir Android Studio:
